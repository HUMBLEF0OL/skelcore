...
export function resolveRolloutPolicy(strategy: "feature-flag", policy: UnknownPolicy): FeatureFlagPolicy;
export function resolveRolloutPolicy(strategy: "staged", policy: UnknownPolicy): StagedPolicy;
export function resolveRolloutPolicy(strategy: RolloutStrategy, policy: UnknownPolicy): CanaryPolicy | FeatureFlagPolicy | StagedPolicy {
    if (strategy === "canary") {
